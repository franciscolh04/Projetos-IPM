...
function createTargets(target_size_x, target_size_y, horizontal_gap, vertical_gap)
{
  ...
  // Set targets in an arbitrarily defined grid
  for (var r = 0; r < GRID_ROWS; r++)
  {
    for (var c = 0; c < GRID_COLUMNS; c++)
    {
      let target_x = 70 + (target_size_x * 0.75 + 0) * c;        // give it some margin from the left border
      let target_y = 50 + (target_size_y / 1.15 + 0) * r;
      
      target_y +=  (c % 2) * (target_size_y * 0.43);       // offset every other row by half the margin

      //if ((r == GRID_ROWS - 1) && (r % 2 != 0)) target_x += target_size_x; // adjust the last target in odd rows
      // Find the appropriate label and ID for this target
      let legendas_index = c + GRID_COLUMNS * r;
      if (legendas_index >= legendas.getRowCount()) break;
      //let target_id = legendas.getNum(legendas_index, 0);  
      //let target_label = legendas.getString(legendas_index, 1);   
      let target_id = order[legendas_index];
      let target_label = legendas.getString(order[legendas_index] - 1, 1);
...
function windowResized() 
{
  if (fullscreen())
  {
    ...
    let target_size_x  = 2.2;
    let target_size_y  = target_size_x;                                // sets the target size (will be converted to cm when passed to createTargets)
    ...
    createTargets(target_size_x * PPCM, target_size_y * PPCM, horizontal_gap * PPCM - 240, vertical_gap * PPCM - 200);
